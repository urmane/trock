...
function _M:init(zone, map, level, data)
	...
	print("[DBG class TileSet generator init completed")
end
...
function _M:roomAlloc(bx, by, bw, bh, rid)
	print("trying room at", bx,by,bw,bh, self.cols, self.rows)
...
function _M:buildTile(tile, bx, by, rid)
	...
	for j = 1, #tile[1] do
		for i = 1, #tile do
			if self.map.room_map[bx * self.block.w + i - 1] and
				self.map.room_map[bx * self.block.w + i - 1][by * self.block.h + j - 1] then
...
function _M:createMap()
	for i = 0, self.map.w - 1 do for j = 0, self.map.h - 1 do
		...
		if self.raw.filler and not self.data.subgenerated then
			c = self.raw.filler(c, i, j, self.map.room_map, self.data)
		end
		-- if self.map.room_map[i][j].symbol then 
		-- 	print (("x=%d, y=%d, is %s"):format(i, j, self.map.room_map[i][j].symbol))
		-- end
...
function _M:makeStairsInside(lev, old_lev, spots)
	...
	if lev < self.zone.max_level or self.data.force_last_stair then
		while true do
			...
			if not self.map:checkEntity(dx, dy, Map.TERRAIN, "block_move") and
			   not self.map.room_map[dx][dy].special then
	...
	-- Put up stairs
	local ux, uy
	if not self.data.skip_up_stair then
		while true do
			ux, uy = rng.range(1, self.map.w - 1), rng.range(1, self.map.h - 1)
			if not self.map:checkEntity(ux, uy, Map.TERRAIN, "block_move") and
			   not self.map.room_map[ux][uy].special then
